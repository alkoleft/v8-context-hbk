> **РЕШЕНИЕ T183 ПРИНЯТО / X1-INT ЕЩЁ НЕ ПРОШЁЛ**
>
> Раздел 1 завершил эксперимент и пользовательский gate. ADR-0012 выбирает X1
> единственным X1-INT кандидатом и отклоняет X1-PROJECTED. Разделы 2–7 теперь
> исполняются по порядку; conditional cutover 5–6 и cleanup 7.2–7.3 запрещены до
> полного X1-INT pass.
...
- [ ] 5.1 Только после полного X1-INT pass перенести каждого runtime-потребителя каталога с десериализованного
  ...
  поведение запросов.
- [ ] 5.2 После canonical cutover удалить только inventory-proven заменённую runtime-модель в куче, дублирующие индексы и
  многоэтапные преобразования; не сохранять совместимую копию.
- [ ] 5.3 Добавить структурную защиту от повторного появления, доказывающую, что
