...
The current provisional Syntax Assistant consumer JSON contract is `schema_version: 8`.
`FR-EXPORT-001` owns the exact record-family shape; `acceptance/baseline.md` records the latest
validated counts and schema-changing task conclusions. The next unchecked implementation tasks
continue the event export split and owner-classification pool before the separate query CLI work.
