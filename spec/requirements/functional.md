...
- Platform type template metadata/application-object types must expose a source-owned open
...
- Type template family derivation starts from each template's alias base, falling back to the
  ...
  reader/index must score direct type-template type-reference links between the unassigned template and
  already derived families. If exactly one family has direct type-template references, the template is
...
- Template parameter labels such as `<Имя документа>` or `<Имя плана видов расчета>` are source
...
- Members and callables owned by platform type templates must preserve source-backed template type
...
- Platform type template metadata/application-object template types are discoverable by source-owned
  family/variant key, and member/callable type references preserve owner-parameter template bindings
  where source evidence links one type template to another.
