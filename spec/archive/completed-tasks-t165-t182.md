# Completed Tasks T165-T182

For active task sequencing, use `../IMPLEMENTATION_TODO.md`.

## T179. Own complete member enumeration and resolver-kind classification
...
## T180. Complete the direct BSL catalog handoff
...
- `HbkBslContextCatalog` now lends typed availability contexts and borrowed
  available-since text. The generic snapshot adapter and downstream concrete
  owned-output boundary share the one upstream fact-id/type-ref/signature
  projection owner.
- Catalog/generic parity, source-absence guards, focused tests, workspace
  check/test, formatting and strict OpenSpec validation pass. Fresh review
  returned no findings. The final Structure impact adds no wrapper, DTO,
  storage/index, fallback or analyzer model. Workspace version is `0.2.2`;
  downstream analyzer integration may now begin.

## T181. Centralize HBK member and callable kind projection
...
## T182. Complete final direct BSL projection handoff
...
## T177. Eliminate duplicate snapshot-interner string ownership
...
## T176. Filter unused owner-edge rows before materialization
...
## T175. Avoid materializing unused signature lines
...
## T174. Bound SQLite type-reference materialization for provider snapshots
...
## T173. Resolve one metadata-selected BSL module member without materializing a module context
...
## T172. Resolve metadata-generated self roles through the HBK template boundary
...
## T171. Add explicit snapshot-backed resolver adapters for worker-safe analyzer lookup
...
## T167. Measure SQLite-first HBK fact snapshot materialization
...
## T166. Expose shcntx query table templates through the QueryLanguage resolver source
...
## T165. Expose core BSL primitive language types through Rust resolver adapters
