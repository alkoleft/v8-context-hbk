...
| `snapshot/mod.rs`: cache re-exports and `HbkFactSnapshotBuildReport.cache_*` fields | materializer and X1 publisher identity check | delete re-exports; rename fields to provider build-input terms | private build-input identity captures full provider SHA-256 and available source-HBK SHA-256 before materialization, then compares both during publication | public API compile tests and ordinary provider-row/source-HBK mutation regressions |
...
| `scripts/benchmark-hbk-snapshot-candidates.sh`, `benchmark-hbk-s83-candidate.py`, `benchmark-hbk-s83-av1.py`, `benchmark-hbk-s83-av2.py` | invoke only the deleted H0/C0/AV1/AV2 examples and oracle | delete with their closed experiment binaries | durable T183 evidence and experiment branches; the AV2 summarizer keeps only its extracted, non-producing `_hbk_s83_av2_evidence_contract.py` validator | retired example/path guard; the frozen summarizer imports the extracted contract; `verify-hbk-s83-semantic-parity.py` remains |
| `scripts/tests/test_hbk_s83_av1.py`, `scripts/tests/test_hbk_s83_av2.py` | tests of the deleted producers; AV2 also contained summarizer regressions | delete producer-coupled suites and extract summary-only coverage | `scripts/tests/test_hbk_s83_av2_summary.py` preserves frozen cardinality, complete/duplicate/missing matrix, ranking and transcript-rejection checks | `python3 -m pytest scripts/tests -q`; exact retired-path guard |
| `syntax-helper-search` dev dependencies `serde`, `serde_json` | only the deleted experiment examples after consumer search | delete if final package consumer search is empty | no replacement; production serialization owners are in their existing crates | manifest/package checks and `rg` consumer search |
| feature `snapshot-experiment` and `HbkFactReadHandle::{experiment_string, experiment_string_id}` | old harness/oracle plus `snapshot-experiment-alloc` wrapper | remove the orphan wrapper/accessors only if allocation tests keep working directly through `snapshot-experiment-alloc` | retained allocator feature/module and canonical read methods | all-features/all-targets tests and allocation regressions |
...
- historical result summarizers, their non-producing frozen validation
  contracts, and `scripts/verify-hbk-s83-semantic-parity.py`;
