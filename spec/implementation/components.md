...
T180 completes the catalog's typed availability boundary: public catalog
consumers receive existing `AvailabilityContext` values and borrowed
available-since text, not raw snapshot string IDs. Stable fact-id, HBK
type-reference and signature projection has one upstream behavior owner reused
by the generic adapter and concrete downstream owned-output boundaries. It
