...
pub mod export {
    ...
    use crate::hbk::book::HbkBook;
    use crate::syntax_helper::PlatformContext;
    ...
    mod tests {
        ...
        use crate::hbk::book::BookLocale;
        use crate::syntax_helper::GlobalMethod;
        ...
        fn records_envelope_json_is_parseable_and_non_empty() {
            let dir = std::env::temp_dir()
                .join(format!("v8-context-hbk-export-test-{}", std::process::id()));
            if dir.exists() {
        ...
        fn exporter_writes_full_canonical_file_set() {
            ...
            for file in &summary.files {
                ...
                serde_json::from_str::<serde_json::Value>(&json).unwrap_or_else(|error| {
                    panic!("{} must be valid JSON: {error}", file.display())
                });
            }

            let metadata = fs::read_to_string(dir.join("metadata.json"))
                .expect("metadata.json must be readable");
